// 五子棋普通提示引擎 v1.0 — 独立状态 / 精确 5 层 / 500 万节点 / 8 秒
//
...
//   3. 迭代加深 Alpha-Beta: 逐层搜索 1..5 层，必胜即返回，
...
//     中间迭代只接受"能赢" (V3 minmax)。常规搜索保持逐层提交，截断兜底由
...
(function (global, factory) {
  ...
  function makeMinmax(onlyThree = false, onlyFour = false) {
    ...
    function helper(evaluator, board, role, depth, cDepth, path, alpha, beta, budget, cache, lastMove, bestSlot) {
      ...
      // 普通引擎: 每轮迭代记录"该深度最优", 迭代结束时提交, 更深迭代优先。
      ...
      for (let d = firstIterationDepth; d <= depth; d += 1) {
        // 普通引擎必须真正完成用户指定的 5 层；不能沿用深度引擎的偶数层门控，
        // 否则 depth=5 实际只会完成 4 层。
  ...
  // 普通档常规搜索和 VCT 均固定在 5 层，保证档位边界清晰。
  ...
  function minmaxSearch(evaluator, board, role, depth, budget, lastMove) {
    const cache = new Map();
    // 普通档所有搜索（包括强制威胁搜索）都受 5 层上限约束；深度档的
    // depth+8 VCT 扩展只存在于 hint.js，不带入本独立引擎。
    const vctDepth = depth;
  ...
  function computeBest(board, color, opts) {
    ...
    // 普通档固定精确 5 层。500 万节点足以覆盖 16 候选上限下的五层主树，
    // 同时与深度档的 3000 万节点预算完全分离。
    const depth = 5;
    const nodeBudget = 5000000;
    ...
    // 普通引擎: 500 万节点硬上限；引擎最多占用 7 秒，给 dispatcher、HTTP
