// 五子棋 AI 普通提示 dispatcher — 独立于深度提示的线程池和引擎文件。
...
const SEARCH_WORKER_PATH = path.join(__dirname, 'hint-normal-worker-search.cjs');
const NUM_WORKERS = 4;
...
function spawnSearchWorker(i) {
  ...
  const markDead = (reason) => {
    if (w._dead) return;
    w._dead = true;
  console.log(`[hint-normal] search-worker#${i} ${reason}; 下次请求时重建`);
  };
...
parentPort.on('message', async (msg) => {
  ...
  console.log(`[hint-normal] 普通搜索开始: id=${msg.id} 棋子=${stones}`);
    ...
    if (workers.some((w) => w._dead)) {
      console.log(`[hint-normal] Lazy SMP 池部分死亡 (${workers.filter((w) => w._dead).length}/${NUM_WORKERS}), 继续用幸存者`);
    }

    console.log(`[hint-normal] Lazy SMP 开始: id=${msg.id} 棋子=${stones} workers=${NUM_WORKERS}`);
    const results = [];
    const listeners = [];
    ...
    for (let i = 0; i < workers.length; i++) {
      ...
      w.on('exit', onExit);
      try {
        w.postMessage({ id: msg.id, board: msg.board, color: msg.color });
      } catch (e) {
    ...
    if (!best) {
      console.log(`[hint-normal] 失败: id=${msg.id} 所有 worker 失败 (${winners}/${NUM_WORKERS})`);
    ...
    console.log("[hint-normal] Lazy SMP 结束: id=" + msg.id + " ms=" + (Date.now() - t0) + " winners=" + winners + "/" + 4 + " → (" + best.x + "," + best.y + ") best.value=" + best.value + " [" + _values + "]" + (incomplete ? " [超时]" : ""));
